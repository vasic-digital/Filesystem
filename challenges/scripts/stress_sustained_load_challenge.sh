...
echo "=== Filesystem Stress Sustained-Load Challenge ==="
echo "  url=$HEALTH_URL dur=${DURATION}s rps=${RPS}"

if [[ -z "$HEALTH_URL" ]]; then
    echo "[1/6] SKIP: FILESYSTEM_HEALTH_URL unset — SKIP-OK: #env-no-target"
    echo "=== Filesystem Stress Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== Filesystem Stress Challenge: PASSED ==="
